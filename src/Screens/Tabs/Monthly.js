...
{/*******************************************************************/}
    ...
    const fetchTransaction = async() => {
        ...
        if (transactions.length > 0){
            ...
            for(i=transactions.length-1; i >= 0; i--) {
                if(i > 0) {
                    if((i > 0) && (transactions[i-1].month != transactions[i].month) || (transactions[i-1].dayOfMonth != 1 && transactions[i].dayOfMonth == 1)) {
    ...
    const fetchMonths = () => {
        if (monthly.length > 0) {
            ...
            }else if(total != currentMonthTrans[0].amount || (total/inc)*100 != currentMonthTrans[0].saved) {

...
// FontAwesome icons
import { faAppleWhole, faBars, faGasPump, faGem, faHouse, faMoneyBill1Wave, faMoneyBillTransfer, faShoppingCart } from '@fortawesome/free-solid-svg-icons';
...
export default function Weekly ({navigation}) {
    ...
    // States from redux
    const {user, budgets} = useSelector(state => state.user);
    const {transactions, weekly} = useSelector(state => state.persistTransaction);
